    ...
    def _generate_rpms_in_file(self, metadata: ImageMetadata) -> Dict:
        ...
        if cachi2_enabled and enabled_repos and len(enabled_repos) > 0:
            ...
            if len(rpms_install) == 0:
                ...
                installed_packages = build.installed_packages
                self._logger.info(f'installed packages: {installed_packages}')
                if len(installed_packages) > 0:
                    ...
                    if len(parents) > 0:
                        ...
                        parent_installed_packages = parent_build.installed_packages
                        self._logger.info(f'parent installed packages: {parent_installed_packages}')
                        # rpms_install = sorted(set(list(installed_packages.keys())) - set(list(parent_installed_packages.keys())))
                        rpms_install = sorted(set(installed_packages) - set(parent_installed_packages))
                    else:
                        rpms_install = list(installed_packages)
                else:
            ...
            if 'packages' not in input_file and 'upgradePackages' not in input_file:
                ...
                input_file['arches'] = list(set(arches) - set(['s390x']))
